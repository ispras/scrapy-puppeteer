...
class DeadContextSpider(scrapy.Spider):
    ...
    async def click_on_navigation(self, response: PuppeteerResponse):
        ...
        click = Click("#__docusaurus > nav > div.navbar__inner > div:nth-child(1) > a:nth-child(3)")
    ...
    async def click_back(self, response: PuppeteerResponse):
        ...
        click = Click("#__docusaurus > nav > div.navbar__inner > div:nth-child(1) > a.navbar__brand > b")
        yield response.follow(
            click,
            callback=self.goto_api,
            errback=self.errback,
            close_page=False
        )
    ...
    async def goto_api(self, response):
        ...
        yield response.follow(GoTo("api/puppeteer.puppeteernode"),
